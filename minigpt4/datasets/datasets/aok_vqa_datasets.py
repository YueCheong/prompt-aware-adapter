...
class AOKVQADataset(VQADataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        ...
        for ann in self.annotation:
            # image_path = os.path.join(self.vis_root, ann["image"].split('/')[-1]) # do not have the key: "image"
            image_file = 'COCO_train2014_{:012}.jpg'.format(ann['image_id'])
            image_path = os.path.join(self.vis_root, image_file)
            if os.path.exists(image_path):
                exist_annotation.append(ann)
        self.annotation = exist_annotation
        print(f'len(exist_annotation):{len(exist_annotation)}')
