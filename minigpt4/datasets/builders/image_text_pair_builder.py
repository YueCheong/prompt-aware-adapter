...
from minigpt4.datasets.datasets.coco_vr_dataset import COCOVRDataset, COCOVREvalDataset # zy add to include coco_vr
from minigpt4.datasets.datasets.mathvista_dataset import MathVistaDataset
...
class MultitaskConversationBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        ...
        logging.info("Building multitask conversation datasets...")
...
class UnnaturalInstructionBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building unnatural instruction datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class LlavaDetailBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building llava detail datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class LlavaReasonBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building llava reason datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class LlavaReasonBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building llava conversation datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class AllRefCOCOBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building all RefCOCO datasets...")
        self.build_processors()
...
class RefVisualGenomeBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building refvg datasets...")
        self.build_processors()
...
class GroundedCaptionBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building flickr grounded caption datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class CaptionToPhraseBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building flickr CaptionToPhrase datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class CaptionToPhraseBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building flickr ObjectToPhrase datasets...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class CCSBUAlignBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building cc_sbu_align datasets...")
        self.build_processors()
...
# zy add coco_vr datast
@registry.register_builder("coco_vr")
class COCOVRBuilder(BaseDatasetBuilder):
    train_dataset_cls = COCOVRDataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/coco_vr/default.yaml",
    }
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building coco_vr datasets...")
        self.build_processors()
        ...
        datasets['train'] = dataset_cls(
            ...
            text_processor=self.text_processors["train"],
            vis_root=build_info.image_path, 
            ann_path=os.path.join(build_info.ann_path, 'train')  # MiniGPT-4/minigpt4/configs/datasets/coco_vr/default.yaml, train/test      
        )       
        return datasets

@registry.register_builder("coco_vr_val")
class COCOVRBuilder(BaseDatasetBuilder):
    train_dataset_cls = COCOVRDataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/coco_vr/default.yaml",
    ...
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building coco_vr_val datasets...")
        self.build_processors()
        ...
        #     text_processor=self.text_processors["train"],
        #     vis_root=build_info.image_path, 
        #     ann_path=os.path.join(build_info.ann_path, 'test')  # MiniGPT-4/minigpt4/configs/datasets/coco_vr/default.yaml, train/test      
        # )
        
        datasets['eval'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            vis_root=build_info.image_path, # MiniGPT-4/minigpt4/configs/datasets/coco_vr/default.yaml, train/test
            ann_path=os.path.join(build_info.ann_path, 'val')        
        )
...
@registry.register_builder("mathvista")
class MathVistaBuilder(BaseDatasetBuilder):
    train_dataset_cls = MathVistaDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/mathvista/default.yaml",
    }

    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building mathvista datasets...")
        self.build_processors()
